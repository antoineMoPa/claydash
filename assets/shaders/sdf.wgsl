...
fn surface_light(point: vec3<f32>, normal: vec3<f32>, view: vec3<f32>, object: Object) -> vec3<f32> {
    ...
    if object.component.w == 1u {
        ...
        let warp = sin(local.y * 5.0 + sin(local.z * 9.0)) * 0.025;
        let rings = length(local.xz + vec2(warp, warp * 0.6)) * 95.0;
        let grain = 0.5 + 0.5 * sin(rings + sin(local.y * 3.0) * 1.5);
        let fibers = 0.5 + 0.5 * sin(local.x * 310.0 + sin(local.y * 7.0) * 2.0);
        color *= (0.55 + 0.45 * grain) * (0.9 + 0.1 * fibers);
    }
    ...
    return color * (0.13 + diffuse * 0.75) * (1.0 - metallic)
